...
class AuthService:
    ...
    async def login(self, body: LoginRequest) -> TokenPairResponse:
        user_doc = await self._user_service.get_user_document_by_email(
            body.email, include_deleted=True
        )
        if not user_doc:
            raise HTTPException(
        ...
        return await self._issue_token_pair(user_doc)
    ...
    async def refresh(self, refresh_token: str) -> TokenPairResponse:
        ...
        if stored.get("_id") != payload.get("jti") or stored.get(
            "userId"
        ) != payload.get("sub"):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid refresh token"
            )
        user = await self._user_service.get_user_document_by_id(
            str(payload.get("sub") or ""), include_deleted=True
        )
        if not user:
            raise HTTPException(
        ...
        return await self._issue_token_pair(user, replaced_token_id=str(stored["_id"]))
    ...
    async def get_user_from_access_token(self, access_token: str) -> UserPublic:
        payload = self._decode_token(
            token=access_token,
    ...
    async def reset_password(
        ...
        updated_user = await self._user_service.reset_password_with_forgot_token(
            token=token,
        ...
        await self._refresh_tokens.revoke_by_user_id(updated_user.id)
        return ResetPasswordResponse(success=True)

    async def invite_user(self, body: InviteUserRequest) -> InviteUserResponse:
        temporary_password = secrets.token_urlsafe(24) + "A1!"
        existing_user = await self._user_service.get_user_document_by_email(
            body.email,
            include_deleted=True,
        )
        is_resend = False
        if existing_user:
            if existing_user.get("deletedAt") is not None:
                created_user = (
                    await self._user_service.reactivate_deleted_user_for_invite(
            ...
            elif existing_user.get("deletedAt") is None:
                created_user = await self._user_service.update_user(
        ...
        stored = await self._user_service.set_invite_token(
            user_id=created_user.id,
            token=invite_token,
            expires_in_minutes=self._settings.invite_token_expire_minutes,
        )
        if not stored:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Unable to prepare user invitation",
            )
        invite_url = self._build_frontend_url(
            "/accept-invite",
            {"token": invite_token},
        )
        sent = self._email_service.send_invite_email(
        ...
        if not sent:
            raise HTTPException(
                ...
                detail="Invite email delivery failed",
            )
    ...
    async def accept_invite(
        ...
        invited_user = await self._user_service.accept_invite_with_token(
            token=token,
            new_password=new_password,
        )
        if not invited_user:
        ...
        await self._refresh_tokens.revoke_by_user_id(invited_user.id)
        return AcceptInviteResponse(success=True)

    async def update_my_profile(
        self, user_id: str, body: SelfProfileUpdateRequest
    ) -> UserPublic:
        try:
            return await self._user_service.update_self_profile(
                user_id=user_id,
                name=body.name,
                email=body.email,
            )
    ...
    async def change_my_password(
        ...
        try:
            await self._user_service.change_self_password(
                user_id=user_id,
        ...
        await self._refresh_tokens.revoke_by_user_id(user_id)
    ...
    async def _issue_token_pair(
        ...
        access_token = jwt.encode(
            access_payload, self._settings.jwt_access_secret, algorithm="HS256"
        )
        refresh_token = jwt.encode(
            refresh_payload, self._settings.jwt_refresh_secret, algorithm="HS256"
        )
        saved = await self._refresh_tokens.create(
        ...
        if replaced_token_id:
            await self._refresh_tokens.revoke_by_id(
                replaced_token_id,
                replaced_by_token_id=str(saved["_id"]),
            )
        public_user = UserService._to_public_user(user_doc)
        return TokenPairResponse(
            accessToken=access_token, refreshToken=refresh_token, user=public_user
        )
    ...
    def _build_frontend_url(self, path: str, query: dict[str, str]) -> str:
