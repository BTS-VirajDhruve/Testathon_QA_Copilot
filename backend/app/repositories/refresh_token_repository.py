...
from app.db.mongo import get_refresh_tokens_collection
...
class RefreshTokenRepository:
    ...
    def get_active_by_hash(self, token_hash: str) -> dict[str, Any] | None:
        now = utc_now()
    ...
    def revoke_by_id(self, token_id: str, replaced_by_token_id: str | None = None) -> bool:
        now = utc_now()
        payload: dict[str, Any] = {"revokedAt": now, "updatedAt": now}
        if replaced_by_token_id:
            payload["replacedByTokenId"] = replaced_by_token_id
        result = self._collection.update_one({"_id": token_id, "revokedAt": None}, {"$set": payload})
    ...
    def revoke_by_user_id(self, user_id: str) -> int:
        now = utc_now()
